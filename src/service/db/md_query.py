...
CREATE_UNIQUE_INDEX_B3_HIST = "CREATE UNIQUE INDEX IF NOT EXISTS idx_b3_hist_ticker_date ON b3_hist (ticker, date)"
DEDUPLICATE_B3_HIST = "DELETE FROM b3_hist WHERE rowid NOT IN (SELECT MIN(rowid) FROM b3_hist GROUP BY ticker, date)"
